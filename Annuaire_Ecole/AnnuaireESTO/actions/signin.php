<?php

    session_start();

    spl_autoload_register(function ($class_name) {
        include '../Classes/'. $class_name . '.php';
    });

    if(isset($_SESSION['user_id'])) {
        if(User::findBy($_SESSION['user_id'])) {
            header('Location: /AnnuaireESTO/');
            die();
        }
    }

    if(!isset($_POST['signin_email']) && !isset($_POST['signin_password'])) {
        header('Location: /AnnuaireESTO/');
        die();
    }

    $email = $_POST['signin_email'];
    $password = $_POST['signin_password'];

    if(($user = User::findBy($email, 'email')) != null) {

        if($password == $user->getPassword()) {

            //Email and password match
            if($user->getAcceptation()) {

                //Email verified
                $_SESSION['user_id'] = $user->getUser_id();
                $_SESSION['nom'] = $user->getNom();
                $_SESSION['email'] = $user->getEmail();
                header('Location: /AnnuaireESTO/home.php');

            } else {

                //Email not verified
                header('Location: /AnnuaireESTO/?'. sha1('not_verified'));

            }


        } else {

            //wrong password
            header('Location: /AnnuaireESTO/?'. sha1('wrong_password'));

        }

    } else {

        //Email not found
        header('Location: /AnnuaireESTO/?'. sha1('not_found'));

    }

?>
